...
<div x-data="imageUpload(@js($config))" {{ $attributes->class('space-y-2') }}>
    <div class="flex flex-wrap items-end justify-between gap-2">
        <x-admin.form-label :label="$label ?? $spec->label" class="mb-0!" />
        <span
    ...
    <div class="max-w-xl">
        <button
            type="button"
            x-on:click="pick()"
            class="{{ $spec->aspect }} group relative block w-full cursor-pointer overflow-hidden rounded-xl border-2 border-dashed border-slate-300 bg-slate-50 transition-colors hover:border-blue-400 dark:border-slate-700 dark:bg-slate-800/50 dark:hover:border-blue-500/50"
            :class="preview && 'border-solid border-slate-200 dark:border-slate-700'"
            aria-label="Choose {{ strtolower($spec->label) }}"
        >
            <template x-if="preview">
                <img :src="preview" alt="" class="absolute inset-0 h-full w-full object-cover" />
            </template>

            <template x-if="!preview">
                <span class="absolute inset-0 flex flex-col items-center justify-center gap-1.5 p-4 text-center text-slate-400">
                    <x-icons.upload class="h-7 w-7" />
                    <span class="text-sm font-semibold text-slate-600 dark:text-slate-300">Click to upload</span>
